...
class Network(object):
    ...
    def _init_configure(self):
        ...
        parser.add_argument('--config',nargs='?',type=str,default='configs/nas_unet/nas_unet_custom.yml',
        ...
        with open(self.args.config) as fp:
            self.cfg = yaml.load(fp,Loader=yaml.Loader)
...
# this was original def _init_device(self):
    ...
    #     # added else block from search_cell.py
    #     else:
    #         self.logger.info('No gpu devices available!, we will use cpu')
    #         self.device = torch.device('cpu')
    #         self.device_id = 0

# this is from search_cell.py
    def _init_device(self):
    ...
    def _init_dataset(self):
        ...
        self.nweight = trainset.CLASS_WEIGHTS
    ...
    def _init_model(self):
        ...
        if torch.cuda.device_count() > 1 and self.cfg['training']['multi_gpus']:
           ...
           # this is commented if errors uncomment it 
...
#self.device = torch.device('cpu')
           ...
           # this code block is new one
            if input.is_cuda:
                input = input.cpu()
            if target.is_cuda:
                target = target.cpu()
# this was original code block which is replaced now
